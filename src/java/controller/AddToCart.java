 ...
 * @author Anne
 */
@WebServlet(name = "AddToCart", urlPatterns = {"/AddToCart", "/RemoveFromCart"})
public class AddToCart extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("/AddToCart".equals(servletPath)) {
            addToCart(request, response);
        } else if ("/RemoveFromCart".equals(servletPath)) {
    ...
    private void removeFromCart(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!Util.isInteger(prId)) {
            ...
            if (product == null) {
                ...
                if (user != null) { //remove product from database cart -> user available

                    Criteria c1 = s.createCriteria(Cart.class);
                    c1.add(Restrictions.eq("user", user));
                    c1.add(Restrictions.eq("product", product));

                    if (!c1.list().isEmpty()) { //product available in cart
                        Cart cart = (Cart) c1.uniqueResult();
                        s.delete(cart);
                        tr.commit();
                        responseObject.addProperty("status", true);
                ...
                } else { // remove product from session cart -> user not available in the http session
                    HttpSession ses = request.getSession();
                    ...
                    if (ses.getAttribute("sessionCart") != null) {
                        ...
                        if (foundedCart != null) {
                            sessionList.remove(foundedCart);
                            responseObject.addProperty("status", true);
                            responseObject.addProperty("message", "Product removed from cart successfully");
                        } else {
                            responseObject.addProperty("message", "Product not found in cart");
